        ...
        <div class="main" id="main">
                ...
                <div class="tab-content mt-3" id="ingredientTabContent">
                    <div class="tab-pane fade show active" id="allIngredients" role="tabpanel">
                        <form action="manage-ingredients" method="post" class="row">
                            ...
                            <div class="col-12">
                                <label class="form-label">Select Existing Ingredient or Create New</label>
                                <select class="form-select" id="ingredientSelect" name="ingredientId">
                                    ...
                                    <c:forEach var="ing" items="${ingredients}">
                                        <option value="${ing.ingredientId}" data-name="${ing.ingredientName}" data-cost="${ing.unitCost}">
                                            ${ing.ingredientName} (Cost: ${ing.unitCost} vnđ/kg)
                                        </option>
                                    </c:forEach>
                                </select>
                            </div>
                    ...
                    <div class="tab-pane fade" id="byDish" role="tabpanel">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select id="filterDishId" class="form-select">
            ...
            <script>
                ...
                document.getElementById("ingredientSelect").addEventListener("change", function () {
                    ...
                    if (ingredientId) {
                        document.getElementById("newIngredientSection").style.display = "none";
                    } else {
                        document.getElementById("newIngredientSection").style.display = "flex";
                    }
                });
                document.addEventListener("DOMContentLoaded", function () {
                    document.getElementById("ingredientSelect").dispatchEvent(new Event("change"));
                });

                document.getElementById("filterDishId").addEventListener("change", function () {
                    ...
                    if (filtered.length === 0) {
                        tbody.innerHTML = "<tr><td colspan='4'>No ingredients found for this dish.</td></tr>";
                    } else {
                        filtered.forEach(function (di, index) {
                            const name = di.ingredientName || "";
                            const id = di.ingredientID != null ? di.ingredientID : "";
                            const cost = di.ingredientCost != null ? di.ingredientCost : "";
                            const qty = di.quantity != null ? di.quantity : "";
                            ...
                            var row = "<tr>" +
                                    ...
                                    "Edit" +
                                    "</button>" +
                                    "<a href='manage-ingredients?action=deleteDishIngredient&dishID=" + selectedDishId + "&ingredientID=" + id + "' " +
                ...
                document.addEventListener("DOMContentLoaded", function () {
                    ...
                    if (tab === "byDish") {
                        ...
                        if (selectedDishID) {
                            const filterSelect = document.getElementById("filterDishId");
                            filterSelect.value = selectedDishID;
                            filterSelect.dispatchEvent(new Event("change"));  // trigger table reload
                        }
                    }
                });
            </script>
