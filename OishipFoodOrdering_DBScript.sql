-- OishipFoodOrdering_DBScript_Ver6.0.sql
...
CREATE TABLE Customer (
    ...
    address NVARCHAR(255) NOT NULL, --Sau này sẽ tạo bảng riêng
...
CREATE TABLE Shipper (
    ...
    vehicle_info NVARCHAR(100) NOT NULL, --Bảng số xe
    status_id INT NOT NULL FOREIGN KEY REFERENCES UserStatus(status_id),
    created_at DATETIME DEFAULT GETDATE()
);
...
CREATE TABLE Category (
    ...
    name NVARCHAR(100) NOT NULL,
    restaurantmanager_id INT NOT NULL FOREIGN KEY REFERENCES RestaurantManager(restaurantmanager_id),
    created_at DATETIME DEFAULT GETDATE()
);
...
CREATE TABLE SupportTicket (
    ...
    shipper_id INT NULL FOREIGN KEY REFERENCES Shipper(shipper_id),
    restaurantmanager_id INT NULL FOREIGN KEY REFERENCES RestaurantManager(restaurantmanager_id),
    ...
    CONSTRAINT chk_support_ticket CHECK (
        (customer_id IS NOT NULL AND shipper_id IS NULL AND restaurantmanager_id IS NULL) OR
        (customer_id IS NULL AND shipper_id IS NOT NULL AND restaurantmanager_id IS NULL) OR
        (customer_id IS NULL AND shipper_id IS NULL AND restaurantmanager_id IS NOT NULL)
    )
);
...
CREATE TABLE RestaurantFee (
    ...
    restaurantmanager_id INT FOREIGN KEY REFERENCES RestaurantManager(restaurantmanager_id),
